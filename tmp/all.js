...
window.x_init_hook_team = function() {
	jQuery('#deal-buy-quantity-input').bind("blur",
	function() {
		var n = parseFloat(jQuery(this).attr('value'), 10);
		var per = parseFloat(jQuery('#deal-per-number').attr('value'), 10);
		var per_min = parseFloat(jQuery('#deal-per-min-number').attr('value'), 10);
		if (n > per && per > 0) {
			n = per
		}
		if (per_min > 0 && n < per_min) {
			n = per_min
		}
		n = isNaN(n) ? '': n;
		jQuery(this).attr('value', n);
		var p = parseFloat(jQuery('#deal-buy-price').html());
		p = isNaN(p) ? 0: p;
		n = isNaN(n) ? 0: n;
		var t = parseFloat(n * p).toFixed(2);
...
X.manage.usermoney = function() {
	var money = jQuery('#user-dialog-input-id').attr('value');
